...
Schema（CURRENCY 占位 = HKD|USD|GBP|EUR|CNY|SGD|AUD|CAD|JPY|KRW 之一）：
{
  "school": string,                         // offer 原文学校名
  "school_zh": string | null,               // 公认中文名；没有就 null
  "program": string,                        // offer 原文专业名
  "program_zh": string | null,
  "degree": string | null,                  // Master / Bachelor / PhD 等
  "degree_zh": string | null,
  "country": string | null,
  "country_zh": string | null,
  "faculty": string | null,
  "faculty_zh": string | null,
  "student_category": string | null,        // Non-local / 本地学生 等
  "language": string | null,                // en / zh / fr 等
  "duration": string | null,                // "1 年" / "30 学分" 等
  "applicant_name": string | null,          // 抄 offer 原文写法，不翻译
  "offer_issue_date": string | null,        // YYYY-MM-DD
  "term_start_text": string | null,         // 入学时间中文描述
  "key_dates": [
    { "type": "accept_deadline"|"deposit_deadline"|"term_start"|"tuition_deadline"|"document_deadline"|"other",
      "date": string, "label": string }
  ],
  "fees": {
    "tuition":     { "amount": number, "currency": CURRENCY, "period": "year"|"term"|"total", "note": string|null, "is_partial": boolean } | null,
    "deposit":     { "amount": number, "currency": CURRENCY, "note": string|null } | null,
    "scholarship": { "amount": number, "currency": CURRENCY, "note": string } | null
  } | null,
  "conditions": [
    { "item": string, "details": string | null,
      "status": "required"|"optional"|"met", "deadline": string | null }
  ],
  "must_do": [
    { "action": string, "details": string | null, "deadline": string | null,
      "priority": "high"|"medium"|"low" }
  ],
  "raw_highlights": [ string ],             // 原文最关键 1-5 句
  "notes": [ string ],                      // 重要备注，每条一句中文
  "info_gaps": [ string ],                  // 固定返回 []，后端反推
  "coverage": {
  ...
  "summary": string                         // 500-800 字中文（详见"叙事解读"段）
}
...
5. 如果整份 offer 上**确实只能找到**裸专业名（极罕见，比如某些非正式 acceptance letter），保留裸名——后续 research 步骤会去补全。**不要**往 info_gaps 写任何东西（info_gaps 由后端反推）。
...
抽取规则（续）：
- 中文名规则（核心：抄原文 + 不会就 null）：
  - school：抄 offer 上的英文 / 原文学校名。
  - school_zh：填该校常用中文名；想不出公认中文名就 null（**绝不机械直译**——后端有 175 校查表会兜底）。
  - program_zh：项目中文译名（官方公布的优先；通用直译次之）。例 "MSc Computer Science" → "计算机科学理学硕士"，"MBA" → "工商管理硕士"。
  - degree_zh: Master → 硕士 / Bachelor → 学士 / PhD → 博士 / Master of Engineering → 工程硕士。
  - country_zh: Hong Kong → 中国香港 / UK → 英国 / US → 美国 / Mainland China → 中国大陆。

中国大学（提到时**绝不机械直译**——例 Central South University = "中南大学" 不是"中央南大学"）：想得出官方中文名就用；想不出就 null。

- **英文规范化**：offer 写全大写的学校 / 专业名（"THE HONG KONG POLYTECHNIC UNIVERSITY"），抽取到 school / program 时规范化为 Title Case。常见缩写（MSc, MBA, PhD, BSc, BA, MEng, LLM, MPhil, USA, UK, HK, MIT, NUS）保留大写。

申请人姓名 (applicant_name)：
- 从 offer 上抽取被录取学生的姓名。常见位置：
  - 信件抬头 "Dear [Name]," / "Dear Mr./Ms. [Name],"
  - "Applicant Name: [Name]"
  - 中文 offer：抬头 "亲爱的 [姓名]" / "[姓名] 同学" / 申请人姓名栏
- **核心规则：原样抄 offer 上的姓名，不要翻译**。
  - offer 写英文 / 拼音（"Kunwei Jiang"）→ 抽 "Kunwei Jiang"，**不要**翻成 "姜坤伟" / "蒋昆维"等任何中文（拼音回译几乎都是错的）。
  - offer 写中文（"姜坤伟"）→ 抽 "姜坤伟"。
  - 双语并存如 "Li Jun (李俊)" → 取**中文** "李俊"（offer 同时写了，说明这是该学生的中文名）。
- 全大写英文名（"LI JUN"）→ 规范化为 Title Case "Li Jun"。
- **去掉敬称**（Mr. / Ms. / Mrs. / Dr. / Prof. / 先生 / 女士 / 同学），只保留姓名本身。
- 泛称 "Dear Student" / "Dear Applicant" → null。
- 找不到就 null，**绝不编造、绝不音译**。
...
币种 (currency) — 严格 ISO 白名单：
- 输出 schema 中已列的 10 个 ISO 代码之一（HKD / USD / GBP / EUR / CNY / SGD / AUD / CAD / JPY / KRW）。任何中文 / 符号 / 非 ISO 写法（"RMB" / "$" / "£" / "港币" / "人民币" 等）都按各自语言映射成 ISO 代码后再输出。
- "$" 单独出现：按学校所在地推断（US → USD / HK → HKD / AU → AUD / CA → CAD / SG → SGD）。
- 不在白名单里的（瑞郎、瑞典克朗等）→ currency 字段填 null（**不要**硬塞最近代码、**不要**往 info_gaps 写）。
...
优先级 2 — offer 只给"按学分单价 + 总学分"或"首期 / 单学期 / 单学分金额"，没有项目总额数字 → tuition = null（**不要自己相乘**），coverage.tuition 据情况标 stated_partial，让后续研究步骤或后处理去补全。

优先级 3 — offer 完全没提学费 → tuition = null，coverage.tuition = "absent"。

**关键判断技巧（防止把优先级 1 误判成 2）**：
...
- 区分点：数字**前后是否给出了"项目总额"**？给了就是优先级 1；只有按学分单价、首期金额，没有总额数字，才是优先级 2。
...
（不再做自动相乘：未明确列出总学费时直接 tuition=null，coverage.tuition 标 stated_partial / absent，后端从 coverage 反推核实项。）
...
输出 JSON 前自检（最常见的丢信息错误，只列三条）：

☐ 1. **每条 condition 的 details 已填完整**——offer 同段落给的学历验证规格、语言成绩具体分数+考试代码+有效期、OR 选项的全部分支，都要逐字进 details。"提交语言成绩"不带分数 / 漏掉任一 OR 选项 = 错误。

☐ 2. **每条 must_do 的 details 已填完整**——上传 / 缴费 URL、首期金额构成（"HK\$X = 首期学费 + 留位费 + 学生活动费"）、逾期后果 / 不退款警告，都要进 details 或 notes[]。

☐ 3. **notes[] 已扫**——offer 所有 "Notes on..." / "Important Notes" / "Authenticity" / "Visa Arrangement" / "Concurrent Registration" / "Refund Policy" / "Caution Money is non-refundable" 段落每条单独成 note，不合并、不省略。
...
export const PROMPT_VERSION = "v15-rebrand-yesletter";
