...
Schema:
{
  ...
  "summary": string                         // 500-800 字中文叙事（详见下方"叙事解读"段落）
}
...
- 如果 offer 上的币种**完全不在**上述 10 个里（极罕见，比如瑞士法郎、瑞典克朗），把 currency 字段设为 null（不要硬塞一个最近的代码）。不要往 info_gaps 写——info_gaps 由后端反推。
...
☐ 12. **summary 已写、达到 500-800 字、且覆盖关键 conditions / must_do / notes 的细节**——summary 是给中介复制粘贴发给学生的工作产物，必须 self-contained。光抽好结构化字段、summary 写两句客套话 = 严重错误。
...
export const PROMPT_VERSION = "v12-resolve-conflicts";
